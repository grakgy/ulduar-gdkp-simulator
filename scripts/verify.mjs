...
try {
  ...
  const mediationFixture = decisionFixture('instigator', 'P127', 'wipe')
  const mediationTeam = [engine.createMember('P127', 'mediation-fixture'), engine.createMember('P085', 'mediation-fixture')]
  const mediationSamples = Array.from({ length: 5000 }, (_, index) => bossDecisionEvents.resolveBossDecision(`mediation-${index}`, mediationFixture, 'compete', mediationTeam))
  const fullMediation = mediationSamples.find((resolution) => resolution.mediationLevel === 'full')
  const partialMediation = mediationSamples.find((resolution) => resolution.mediationLevel === 'partial')
  const unmediatedConflict = mediationSamples.find((resolution) => resolution.tag === '火药味失控' && !resolution.preMediation)
  const mediationStagesValid = fullMediation?.preMediation?.tag === '火药味失控'
    && fullMediation.preMediation.moraleDelta === -10
    && fullMediation.moraleDelta === undefined
    && fullMediation.modifiers === undefined
    && partialMediation?.preMediation?.tag === '火药味失控'
    && partialMediation.preMediation.moraleDelta === -10
    && partialMediation.moraleDelta === -5
    && partialMediation.modifiers?.teamOutputMultiplier === .975
    && unmediatedConflict?.moraleDelta === -10
    && unmediatedConflict.modifiers?.teamOutputMultiplier === .95
